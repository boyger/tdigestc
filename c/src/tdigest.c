...
double td_avg(td_histogram_t *h) {
     return td_total_sum(h) / td_total_count(h);
}
...
void td_add(td_histogram_t *h, double mean, double count) {
     ...
     if (isnan(h->min) || mean < h->min) {
          h->min = mean;
     }
     if (isnan(h->max) || mean > h->max) {
          h->max = mean;
     }
     h->unmerged_nodes++;
...
int td_as_bytes(td_histogram_t *h, unsigned char **buffer) {
     merge(h);
     int header_size = sizeof(td_histogram_t);
     int array_size = sizeof(node_t) * h->merged_nodes;
     *buffer = malloc(header_size + array_size);
     if (!(*buffer)) {
          // malloc failed
          return 0; 
     }
     ...
     if (array_size > 0) {
          memcpy(*buffer + header_size, h->nodes, array_size);
     }
     return header_size + array_size;
}

td_histogram_t *td_from_bytes(unsigned char *buffer, int size) {
     int header_size = sizeof(td_histogram_t);
     if (size < header_size) {
          //data size less then struct size
          return NULL;
     }
     double compression;
     memcpy(&compression, buffer, sizeof(double));
     td_histogram_t *h = td_new(compression);
     memcpy(h, buffer, header_size);
     int array_size = sizeof(node_t) * h->merged_nodes;
     if (size < header_size + array_size) {
          //data size less then struct szie plus nodes size
          return NULL;
     }
     memcpy(h->nodes, buffer + sizeof(td_histogram_t), sizeof(node_t) * h->merged_nodes);
     return h;
}
